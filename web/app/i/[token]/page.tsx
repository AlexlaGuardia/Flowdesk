    ...
    <div className="min-h-screen bg-white py-12">
      <div className="max-w-2xl mx-auto px-6">
        <div className="mb-8 pb-6 border-b border-gray-200">
          <p className="text-sm font-semibold tracking-wide" style={{ color }}>{data.freelancer.business_name || data.freelancer.name}</p>
          <h1 className="text-2xl font-bold text-gray-900 mt-2">Invoice {data.invoice_number}</h1>
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="text-left px-5 py-3 text-gray-500 uppercase text-xs tracking-wider font-semibold">Description</th>
                <th className="text-right px-5 py-3 text-gray-500 uppercase text-xs tracking-wider font-semibold">Qty</th>
                <th className="text-right px-5 py-3 text-gray-500 uppercase text-xs tracking-wider font-semibold">Rate</th>
                <th className="text-right px-5 py-3 text-gray-500 uppercase text-xs tracking-wider font-semibold">Amount</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {data.items?.map((item, i) => (
                <tr key={i}>
                  ...
                  <td className="px-5 py-3 text-right text-gray-700 font-mono">{item.quantity}</td>
                  <td className="px-5 py-3 text-right text-gray-700 font-mono">${item.unit_price.toFixed(2)}</td>
                  <td className="px-5 py-3 text-right text-gray-900 font-medium font-mono">${item.amount.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="bg-gray-50">
                ...
                <td className="px-5 py-3 text-right text-xl font-bold font-mono text-gray-900">${data.total.toFixed(2)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        {data.status === "paid" ? (
          ...
          <div className="mt-6">
            <a
              ...
              className="block w-full py-3 text-center text-white font-medium rounded-lg transition-colors hover:opacity-90"
